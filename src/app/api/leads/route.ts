...
import { linkLeadToCustomer } from "@/lib/customer-journey";
...
const leadSchema = z.object({
  ...
  visitorId: z.string().uuid().optional(), sessionId: z.string().uuid().optional(),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const customerId = await linkLeadToCustomer({ db, leadId: id, name: lead.name, company: lead.company, email: lead.email, country: lead.country, visitorId: lead.visitorId, sessionId: lead.sessionId, occurredAt: now });
    await db.execute({ sql: "INSERT INTO audit_logs (id,action,entity_type,entity_id,details,created_at) VALUES (?,?,?,?,?,?)", args: [randomUUID(), "lead_created", "lead", id, JSON.stringify({ sourcePath: lead.sourcePath, customerId }), now] });
    return NextResponse.json({ success: true, data: { id, customerId }, requestId }, { status: 201 });
