...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> { const { locale, slug } = await params; return { robots: { index: false, follow: true }, alternates: { canonical: `/${locale}/products/${slug}` } }; }

export default async function ProductPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale, slug } = await params;
  if (!siteConfig.supportedLocales.includes(locale as Locale)) notFound();
  if (locale === "es") { const redirects: Record<string, string> = { "edj-fire-pump-set": "sistema-bomba-incendio-edj", "diesel-jockey-pump-set": "bomba-diesel-contra-incendio", "electric-jockey-pump-set": "bomba-electrica-contra-incendio", "long-shaft-fire-pump": "bomba-incendio-eje-largo" }; if (redirects[slug]) permanentRedirect(`/es/productos/${redirects[slug]}`); }
