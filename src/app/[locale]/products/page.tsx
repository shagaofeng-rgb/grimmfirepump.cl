...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> { const { locale } = await params; return { robots: locale === "es" ? { index: false, follow: true } : { index: false, follow: true }, alternates: { canonical: `/${locale}/products` } }; }
