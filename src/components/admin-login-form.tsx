...
export function AdminLoginForm() {
  ...
  async function login(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/admin/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: form.get("email"), password: form.get("password"), remember: form.get("remember") === "on" }) });
  ...
  return <form onSubmit={login} className="admin-form" noValidate>
    ...
    <label>账号<input name="email" type="email" required autoComplete="username" aria-describedby={error ? "login-error" : undefined} /></label>
